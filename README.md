...
| Product images show as broken links | The seed script maps each product to `https://retailhubphotos.blob.core.windows.net/product-images/{slug}.jpg` (full) and `https://retailhubphotos.blob.core.windows.net/product-images/{slug}-thumb.jpg` (thumbnail). Upload blobs with filenames that match both conventions, or update `@BaseUrl`/filename convention in `ProductImage.Test.Seed.sql` and republish. To generate one image + one thumbnail per product, run `tools/Generate-ProductBlobPlaceholders.ps1 -Mode Pexels` with `PEXELS_API_KEY` set. Output: `tools/blob-product-images/` (gitignored). |
