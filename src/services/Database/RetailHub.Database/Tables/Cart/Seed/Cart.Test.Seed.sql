...
USING (
    ...
    ) AS V ([Uid], [CreatedOn], [DeletedOn], [UserId], [AnonymousKey])
) AS S
    ON T.[Uid] = S.[Uid]
WHEN MATCHED THEN
    UPDATE SET
...
WHEN NOT MATCHED THEN
    INSERT ([Uid], [CreatedOn], [DeletedOn], [UserId], [AnonymousKey])
    VALUES (S.[Uid], S.[CreatedOn], S.[DeletedOn], S.[UserId], S.[AnonymousKey]);
